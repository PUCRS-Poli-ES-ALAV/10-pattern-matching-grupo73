...
#define Q 9999971
#define R 26  // 26 letras + 10 dígitos
#define A 127
#define MOD 128
...
long hash_dos_guris(char* s, int m)
{
    long h = 0;
    char* s1 = s;
    for (int i = 0; i < m; i++)
...
long rolling_hash_dos_guris(char* s, int m)
{
    long h = 0;
    char* s1 = s;
    for (int i = 0; i < m; i++)
    {
        h += (*s * (long)pow((double)A, (double)m-i));
        s1++;
    }
    return h % MOD;
} 
...
int search(char* txt, char* pat)
{
    int M = strlen(pat);
    int N = strlen(txt);
    long patHash = hash_dos_guris(pat, M);
    //printf("%s %ld\n","Hash do pattern: ",patHash);
    for (int i = 0; i <= N - M; i++)
    {
        long txtHash = hash_dos_guris(txt+i, M);
        //printf("%s %ld\n","Hash da substring: ",txtHash);
        if (patHash == txtHash)
        {
            char* txt_teste = txt + i;
            char* pat_teste = pat;
            char flag = 1;
            //printf("%s %d\n","Testando",i);
            int count = 0;
            while (txt_teste != '\0' && pat_teste != '\0' && count < M)
            {
                if (*txt_teste != *pat_teste)
                {
                    //printf("%s %c %c %c %c %c %d\n","Diferentes:",*txt_teste,*(pat_teste-2), *(pat_teste-1), *pat_teste, *(pat_teste+1), i);
                    flag = 0;
                    break;
                }
                pat_teste++;
...
int rolling_search(char* txt, char* pat)
{
    int M = strlen(pat);
    int N = strlen(txt);
    long patHash = rolling_hash_dos_guris(pat, M);
    long txtHash = rolling_hash_dos_guris(txt, M);
    printf("%s %ld\n","Hash do pattern: ",patHash);
    printf("%s %ld\n","Hash da sub: ",txtHash);
    for (int i = 0; i <= N - M; i++)
    {
        ...
        txtHash -= (*txt * (long)pow((double)A, (double)M-1));
        txtHash = txtHash  % MOD;
        txtHash *= A;
        txtHash += (*(txt+M+i+1) * (long)pow((double)A, 0.0));
        txtHash = txtHash  % MOD;
        printf("%s %ld\n","Hash deslocado da sub: ",txtHash);
        printf("%s %ld\n","Hash verdadeiro da sub: ",rolling_hash_dos_guris(txt+i+1, M));
    }
    //printf("%s %ld\n","Hash do pattern: ",patHash);
    return -1;
}
...
int main()
{
    ...
    //printf("%ld\n", hash_dos_guris("Morbi eu elit eget ex hendrerit feugiat in ut odio.", strlen("Morbi eu elit eget ex hendrerit feugiat in ut odio.")));
    //printf("%ld\n", hash_dos_guris(lorem_ipsum_20 + 340, strlen("Morbi eu elit eget ex hendrerit feugiat in ut odio.")));
    //printf("%s\n",lorem_ipsum_20 + 340);
    long momoi = patternMatchingBasico(lorem_ipsum_20,"Morbi eu elit eget ex hendrerit feugiat in ut odio.");
    clock_t end = clock();
    ...
    //long basic = rolling_search(lorem_ipsum_20,"Morbi eu elit eget ex hendrerit feugiat in ut odio.");
    long basic = rolling_search("asdasdasdasdasd","das");
    end = clock();
